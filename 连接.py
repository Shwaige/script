...
class Sqldriver:
    ...
    # 插入数据
    def insert(self, fid, fdate, fproduct_id, fcost_time, fstatus, flog_content, fbus_type, fbus_name, fpmoney,
               fomoney):
        try:
            # 连接数据库
            self.connect()
            # sql命令
            sql = "insert into test100000 (fid,fdate,fproduct_id,fcost_time,fstatus,flog_content,fbus_type,fbus_name,fpmoney,fomoney)" \
