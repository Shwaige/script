...
def b():
     ...
     print(r.get("refreshToken"))
     h = {"refreshToken":refreshToken_value}
     h =json.dumps(h)
...
def d():
    ...
    if  r.get("access_token") is not  None:
        accessToken_value = str(r.get("access_token"),encoding="utf-8")
        if   headers.get("access_token") == accessToken_value:
            print(headers.get("access_token"))
            return {"result": "距离accessToken过期还剩"+str(r.ttl("access_token"))+"s"}
        if params.get("access_token") ==accessToken_value:
            return {"result": "距离accessToken过期还剩"+str(r.ttl("access_token"))+"s"}
        if   data  :
            data1 = json.loads(data)
            if data1.get("access_token")==accessToken_value:
              return  {"result": "距离accessToken过期还剩"+str(r.ttl("access_token"))+"s"}
            else:
